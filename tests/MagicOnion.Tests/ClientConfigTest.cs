...
namespace MagicOnion.Tests
{
    ...
    public class ClientConfigTest
    {
        ...
        */
        ...
        {
            (client.AsDynamic().host as string).IsNull();

            var hostChange = client.WithHost("newhost");
            (hostChange.AsDynamic().host as string).Is("newhost");

            var serverContext = await hostChange.ReturnContext();
            serverContext.Host.Is("newhost");
        }
        ...
        public async Task WithDeadline()
        {
            ...
            context.Deadline.ToString().Is(now.ToString()); // almost same:)
        }
        ...
        public async Task WithHeaders()
        {
            ...
            opt = (CallOptions)change.AsDynamic().option;
            opt.Headers[0].Key.Is("test");
            opt.Headers[0].Value.Is("aaaaa");

            var context = await change.ReturnContext();
            context.RequestHeaders["test"].Is("aaaaa");
        }
        ...
        public void WithOption()
        {
            ...
            opt = (CallOptions)change.AsDynamic().option;
            opt.WriteOptions.Flags.Is(WriteFlags.BufferHint);
        }
