...
public class MagicOnionTypeInfo : IEquatable<MagicOnionTypeInfo>
{
    ...
    {
        public static MagicOnionTypeInfo System_Void { get; } = new("System", "Void", SubType.ValueType);
        public static MagicOnionTypeInfo System_String { get; } = new("System", "String");
        public static MagicOnionTypeInfo System_Boolean { get; } = new("System", "Boolean", SubType.ValueType);
        public static MagicOnionTypeInfo MessagePack_Nil { get; } = new("MessagePack", "Nil", SubType.ValueType);
        public static MagicOnionTypeInfo System_Threading_CancellationToken { get; } = new("System.Threading", "CancellationToken", SubType.ValueType);
        public static MagicOnionTypeInfo System_Threading_Tasks_Task { get; } = new("System.Threading.Tasks", "Task");
        public static MagicOnionTypeInfo System_Threading_Tasks_ValueTask { get; } = new("System.Threading.Tasks", "ValueTask", SubType.ValueType);
        public static MagicOnionTypeInfo MagicOnion_UnaryResult { get; } = new("MagicOnion", "UnaryResult", SubType.ValueType);
    }
    ...
    public MagicOnionTypeInfo GetGenericTypeDefinition()
    {
        ...
        return Create(Namespace, Name, Array.Empty<MagicOnionTypeInfo>(), IsValueType);
    }
    ...
    public string FullName => fullName ??= ToDisplayName(DisplayNameFormat.FullyQualified);
    public string FullNameOpenType => fullNameOpenType ??= ToDisplayName(DisplayNameFormat.FullyQualified | DisplayNameFormat.OpenGenerics);
    ...
    public static MagicOnionTypeInfo CreateEnum(string @namespace, string name, MagicOnionTypeInfo underlyingType)
        => new(@namespace, name, SubType.Enum, arrayRank: 0, genericArguments: Array.Empty<MagicOnionTypeInfo>(), underlyingType: underlyingType);
    ...
    public static MagicOnionTypeInfo CreateFromSymbol(ITypeSymbol symbol)
    {
        ...
        if (finalSymbol is INamedTypeSymbol namedTypeSymbol)
        {
            ...
            var typeArguments = namedTypeSymbol.TypeArguments.Select(CreateFromSymbol).ToArray();
        ...
        else if (isArray && finalSymbol is IArrayTypeSymbol)
        {
            ...
            return CreateArray(Create(@namespace, name, Array.Empty<MagicOnionTypeInfo>(), false), arrayRank);
        }
        else
        {
    ...
    public bool Equals(MagicOnionTypeInfo? other)
    {
        ...
        return Namespace == other.Namespace &&
               ...
               Equals(UnderlyingType, other.UnderlyingType);
    }
    ...
    public override int GetHashCode() => (FullName, subType, ElementType, UnderlyingType).GetHashCode();
}
