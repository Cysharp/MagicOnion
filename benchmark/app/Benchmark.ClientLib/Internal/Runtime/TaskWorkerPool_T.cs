...
{
    public class TaskWorkerPool<TRequest, TReply> : IDisposable
    {
        ...
        private readonly Channel<Func<int, TRequest, CancellationToken, Task<TReply>>> _channel;
        ...
        public TaskWorkerPool(int workerCount, int channelSize, CancellationToken ct)
        {
            ...
            _channel = System.Threading.Channels.Channel.CreateBounded<Func<int, TRequest, CancellationToken, Task<TReply>>>(new BoundedChannelOptions(channelSize)
            {
                SingleReader = false,
                SingleWriter = true,
        ...
        public void RunWorkers(Func<int, TRequest, CancellationToken, Task<TReply>> action, TRequest request, CancellationToken ct)
        {
            if (action == null)
        ...
        /// </summary>
        /// <returns></returns>
        private void RunCore(Func<int, TRequest, CancellationToken, Task<TReply>> action, TRequest request, CancellationToken ct)
        {
            // write
            Task.Run(async () =>
            {
